...
function getTargetForwardSpeed(targetType, wave) {
    ...
    else if (targetType === 'Armoured target') multiplier = 0.70;
    else if (targetType === 'Shielded target') multiplier = 0.80;
    else if (targetType === 'Explosive target' || targetType === 'Red Target') multiplier = 0.90;
    else if (targetType === 'BOSS') {
        multiplier = Math.min(0.70, 0.55 + ((wave - 5) / 5) * 0.03);
    }

    const finalSpeed = waveSpeed * multiplier;

    // Scale the config values to the game engine units (0.48 -> 2.6)
    const engineSpeed = finalSpeed * 5.416;

    // 50 units total travel distance from spawn (-40) to end (10)
    const travelTime = 50 / engineSpeed;

    // Assign debug information globally
    window.debugSpeedInfo = {
        wave: wave,
        baseSpeed: waveSpeed.toFixed(3),
        targetType: targetType,
        multiplier: multiplier.toFixed(2),
        finalSpeed: finalSpeed.toFixed(3),
...
function updateGameplay(delta) {
    ...
    if (window.debugSpeedInfo) {
        const debugLabel = document.getElementById('debug-label');
        if (debugLabel) {
            debugLabel.innerText = `Wave: ${window.debugSpeedInfo.wave}
Base speed: ${window.debugSpeedInfo.baseSpeed}
Multiplier: ${window.debugSpeedInfo.multiplier}x
Final speed: ${window.debugSpeedInfo.finalSpeed}
Travel time: ${window.debugSpeedInfo.travelTime} seconds`;
        }
    }
